...
class RLHyperparameter:
    ...
    def evaluate_generalization(self, params, run_id):
        ...
        gen_gap = train_mean_reward - test_mean_reward
    ...
    def build_dataset(self, save_path=None, num_seeds=1):
        ...
        num_seeds : int
            Number of different seeds to evaluate each hyperparameter combination with
        """
        ...
        for i, params in enumerate(tqdm(sampled_params, desc="Evaluating hyperparameter combinations")):
            ...
            # Run the same hyperparameter combination with multiple seeds
            for seed_rep in range(num_seeds):
                # Generate a random seed and use it
                seed = random.randint(0, 100000)
                np.random.seed(seed)
                self.seed = seed
                print(f"  Seed {seed_rep+1}/{num_seeds}: Using random seed {self.seed}")

                perf_metrics = self.evaluate_generalization(params, run_id=i * num_seeds + seed_rep)
                result = {**params, **perf_metrics, 'seed_repetition': seed_rep, 'seed_value': seed}
